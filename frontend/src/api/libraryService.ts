...
export const getLibraries = async (): Promise<Library[]> => {
    try {
        const response = await api.get<PaginatedResponse<Library>>(
            "/rooms/libraries/"
        );
...
export const getLibraryFloors = async (libraryId: number): Promise<Floor[]> => {
    try {
        const response = await api.get<PaginatedResponse<Floor> | Floor[]>(
            `/rooms/floors/`,
            {
...
export const createReservation = async (reservationData: {
    ...
    try {
        ...
        const roomResponse = await api.get(
            `/rooms/rooms/${reservationData.room_id}/`
        );
...
export const cancelReservation = async (
    ...
    try {
        const response = await api.patch(
            `/rooms/reservations/${reservationId}/`,
            {
                status: "cancelled",
            }
        );
...
export const getLibraryMaterials = async (
    libraryId: number
): Promise<Material[]> => {
    try {
        const response = await api.get(
            `/rooms/libraries/${libraryId}/materials/`
        );
